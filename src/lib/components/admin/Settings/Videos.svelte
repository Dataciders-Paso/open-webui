<script lang="ts">
	...
	let config = null;
	let videoGenerationConfig = null;
	...
	const saveHandler = async () => {
		...
		await updateVideoGenerationConfig(localStorage.token, videoGenerationConfig).catch((error) => {
			toast.error(`${error}`);
			loading = false;
			return null;
		});
	...
	onMount(async () => {
		if ($user.role === 'admin') {
			...
			const videoConfigRes = await getVideoGenerationConfig(localStorage.token).catch((error) => {
				toast.error(`${error}`);
				return null;
			});

			if (videoConfigRes) {
				videoGenerationConfig = videoConfigRes;
			}
		}
	});
</script>
...
>
	<div class=" space-y-3 overflow-y-scroll scrollbar-hidden pr-2">
		{#if config && videoGenerationConfig}
			<div>
				<div class=" mb-1 text-sm font-medium">{$i18n.t('Video Settings')}</div>

				<div>
					<div class=" py-1 flex w-full justify-between">
						<div class=" self-center text-xs font-medium">
							{$i18n.t('Video Generation (Experimental)')}
						</div>
						...
						<div class="px-1">
							<Switch
								...
								on:change={(e) => {
									...
									if (enabled) {
										if (config.engine === 'comfyui' && config.comfyui.COMFYUI_BASE_URL === '') {
				...
				{#if config.enabled}
					<div class=" py-1 flex w-full justify-between">
						<div class=" self-center text-xs font-medium">{$i18n.t('Video Prompt Generation')}</div>
						<div class="px-1">
				...
				<div class=" py-1 flex w-full justify-between">
					<div class=" self-center text-xs font-medium">{$i18n.t('Video Generation Engine')}</div>
					...
					{#if config.comfyui.COMFYUI_WORKFLOW}
						<div class="">
							...
							<div class="mt-2 text-xs text-right text-gray-400 dark:text-gray-500">
								{$i18n.t('*Prompt node ID(s) are required for video generation')}
							</div>
						</div>
					{/if}
			...
			{#if config?.enabled}
								...
								<div class="flex-1">
									<Tooltip content={$i18n.t('Enter Model ID')} placement="top-start">
										<input
											list="model-list"
											class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
											bind:value={videoGenerationConfig.MODEL}
				...
				<div>
					<div class=" mb-2.5 text-sm font-medium">{$i18n.t('Set Video Size')}</div>
					<div class="flex w-full">
						<div class="flex-1 mr-2">
							<Tooltip content={$i18n.t('Enter Video Size (e.g. 512x512)')} placement="top-start">
								<input
									class="w-full rounded-lg py-2 px-4 text-sm bg-gray-50 dark:text-gray-300 dark:bg-gray-850 outline-hidden"
									placeholder={$i18n.t('Enter Video Size (e.g. 512x512)')}
									bind:value={videoGenerationConfig.VIDEO_SIZE}
									required
								/>
							</Tooltip>
						</div>
						...
						<div class="flex-1 mr-2">
							<Tooltip content={$i18n.t('Enter Number of Steps (e.g. 50)')} placement="top-start">
								<input
									...
									bind:value={videoGenerationConfig.VIDEO_STEPS}
									required
								/>
							</Tooltip>
						</div>
